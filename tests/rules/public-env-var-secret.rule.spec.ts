...
it('detects a secret-named VITE_ env var', () => {
      const findings = publicEnvVarSecretRule.check('file.ts', 'const apiKey = import.meta.env.VITE_API_KEY;');

      expect(findings).toHaveLength(1);
});

it('detects a secret-named REACT_APP_ env var', () => {
      const findings = publicEnvVarSecretRule.check('file.ts', 'const token = process.env.REACT_APP_AUTH_TOKEN;');

      expect(findings).toHaveLength(1);
});

it('does not flag a public env var without a sensitive name', () => {
      const findings = publicEnvVarSecretRule.check('file.ts', 'const apiUrl = process.env.NEXT_PUBLIC_API_URL;');
