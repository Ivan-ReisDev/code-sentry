...
it('does not flag cors() with a specific origin', () => {
      const findings = permissiveCorsRule.check('file.js', "app.use(cors({ origin: 'https://example.com' }));");

      expect(findings).toHaveLength(0);
});

it('detects setting the Access-Control-Allow-Origin header to "*"', () => {
      const findings = permissiveCorsRule.check('file.js', "res.setHeader('Access-Control-Allow-Origin', '*');");
