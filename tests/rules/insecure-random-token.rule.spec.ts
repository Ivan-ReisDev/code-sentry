...
it('detects a token generated with Math.random()', () => {
      const findings = insecureRandomTokenRule.check('file.js', 'const token = Math.random().toString(36);');
...
it('detects a reset token hashed directly from a getTime() call', () => {
      const findings = insecureRandomTokenRule.check('file.js', 'const resetToken = md5(email + now.getTime());');

      expect(findings).toHaveLength(1);
});

it('does not flag a hash-derived token when the input is not a predictable timestamp', () => {
      const findings = insecureRandomTokenRule.check('file.js', 'const token = sha256(crypto.randomBytes(32));');
