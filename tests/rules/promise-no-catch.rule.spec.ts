...
it('does not flag a .then() followed by .catch()', () => {
      const findings = promiseNoCatchRule.check('file.js', 'promise.then(handleSuccess).catch(handleError);');

      expect(findings).toHaveLength(0);
});

it('does not flag .then() called with a rejection handler as the second argument', () => {
      const findings = promiseNoCatchRule.check('file.js', 'promise.then(handleSuccess, handleError);');

      expect(findings).toHaveLength(0);
});

it('does not flag a longer chain that eventually reaches .catch()', () => {
      const findings = promiseNoCatchRule.check('file.js', 'promise.then(a).then(b).catch(c);');
